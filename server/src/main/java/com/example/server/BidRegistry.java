...
public class BidRegistry {
     ...
     */
    public SocketBroadcaster getOrCreate(String auctionId) {
     ...
     * Trả về null nếu chưa có client nào join phiên (không cần broadcast).
     */
    public SocketBroadcaster get(String auctionId) {
        return map.get(auctionId);
    }

    /**
     * Xóa Broadcaster khi phiên kết thúc (giải phóng bộ nhớ).
     */
    public void remove(String auctionId) {
        map.remove(auctionId);
        System.out.println("[BidRegistry] Đã xóa broadcaster của phiên: " + auctionId);
    }
